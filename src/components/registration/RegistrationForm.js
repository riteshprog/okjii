...
export default class RegistrationForm extends Component {
  ...
  getAddressFromPincode = async (pincode) => {
    return await Axios.post(process.env.REACT_APP_API_URL + `/google/get-addr`, { pincode }).then(({data})=> data );
  }
    ...
    if(name == 'pincode' && value.length == 6){
      ...
      if(pincodeObj.district && pincodeObj.state){
        ...
        if(!preRegistrationAllowedDistricts.includes(district)){
          ...
          console.log(`Pincode should be of ${preRegistrationAllowedDistricts.join(', ')}`);
        }else{
  ...
  handleSubmit = (e) => {
    ...
    const { pincode, fullName, mobileNumber, email } = this.state.preRegistraionData;
    if(pincodeAllowed && isMobileVerified){
      const data = { pincode, fullName, mobileNumber, isMobileVerified, email, district, state }
      Axios.post(process.env.REACT_APP_API_URL + '/pre-registraion', data).then(({data})=>{
        if(data.status){
  ...
  render() {
    return (
      <div>
           {!this.state.showOffer?(<MDBCard className="okjii-registration">
           <MDBCardBody>
              <form>
                <input
                ...
                />
                <MDBBtn className="btn btn-getotp" type="submit" onClick={this.handleSendOtp}>
                    Get OTP
                  </MDBBtn>
...
>>>>>>> 58de028074108435bc837ed894f6d87fed08e9c3
                ...
                /> 
                {this.state.showPincodeMark?<div className='p-2'>
...
>>>>>>> 58de028074108435bc837ed894f6d87fed08e9c3
          ...
          <div className='offer-wrapper'>
            <div className='f2'>
              <span className='offer-title'>{this.state.offer.title}</span>
              <span>{this.state.offer.howToApply.map(step=><div className='df aic mtb-10'><MDBIcon className='green-text' icon="certificate" /><li className='lsn ml-2'>{step}</li></div>)}</span>
            </div>
            <div className='f1'>
